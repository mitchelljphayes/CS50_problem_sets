...
candidate merge_sort (candidate candidates);
candidates merge(candidate A, candidate B, int A_size, int B_size);
...
void print_winner(void)
{
    candidates = merge_sort(candidates);
...
candidate merge_sort (candidate candidates)
{
    int array_size = sizeof(candidates) / 1; //find array length
    if (array_size == 1);
    {
        return;
    }
    ...
    merge(array_A, array_B, A_size, B_size);
}
