...
{
    bool digit_check = true; // creating a boolean for verifying digits only in the key need to be here to pass out of if statement.
    ...
    {
        unsigned int key = atoi(argv[1]); // converts argv string to int
        ...
        {
            char cipherchar = encipher(s[i], key);
